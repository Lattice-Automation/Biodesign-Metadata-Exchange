    ...
    def execute_operation(self, operation_name, **operation_args):
        ...
        Args:
            ...
            **operation_args: Keyword arguments specific to the operation
                - comments: Optional list of comments for this operation
                - status: Optional status string for this operation
                - Other operation-specific arguments
        ...
        # Extract comments and status from operation_args if present (these are not operation-specific args)
        comments = operation_args.pop("comments", None)
        status = operation_args.pop("status", "")
        ...
        if operation_name == "OPEN_PROTEIN" or \
            ...
            operation.execute(self.state, is_protein=True, comments=comments, status=status)
        else:
            operation.execute(self.state, comments=comments, status=status)
