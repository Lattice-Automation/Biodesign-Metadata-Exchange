...
def execute_use_case(cad, use_case_id):
    ...
    for operation in use_case["operations"]:
        ...
        operation_args = operation["operation_arguments"].copy()  # Make a copy to avoid modifying original
        
        # Extract comments and status if present in the operation definition (at the operation level, not in operation_arguments)
        if "comments" in operation:
            operation_args["comments"] = operation["comments"]
        if "status" in operation:
            operation_args["status"] = operation["status"]
