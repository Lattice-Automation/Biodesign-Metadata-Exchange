"""
OpenOperation - Opens an existing PDB protein file.
...
Example:
    OPEN_PROTEIN file_name=my_design

Notes:
    - The file must exist in the library directory
    - The file must be a valid PDB file
...
class OpenProteinOperation(IOperation):
    def __init__(self):
        super().__init__()
        self.file_name = None
    ...
    def validate_and_set_args(self, **args):
        """
        Validates and sets the arguments required for opening a protein file.
        ...
        if not file_name:
            raise ValueError("OPEN_PROTEIN operation requires a file_name")
        self.file_name = file_name
        if not os.path.exists(f"library/{self.file_name}.pdb"):
            raise ValueError(f"A file with the name {self.file_name}.pdb doesn't exist")
    ...
    def get_operation_details(self):
        """
        Returns the details of the protein open operation.
        
        This method provides a dictionary containing all the parameters
        that define this specific protein open operation.
